...
export class Sway extends Service {
    ...
    constructor() {
        ...
        this._monitors_inter = new Map();
        this._workspaces = new Map();
        this._workspaces_inter = new Map();
        this._clients = new Map();
        this._clients_inter = new Map();

        this._initial_tree_parse = true;
    ...
    _handleTreeMessage(node) {
        switch (node.type) {
            case 'root':
                // We only want to update at once when we have the final
                // data
                this._workspaces_inter = new Map();
                this._clients_inter = new Map();
                this._monitors_inter = new Map();
                ...
                this._workspaces = this._workspaces_inter;
                this._clients = this._clients_inter;
                this._monitors = this._monitors_inter;
                this._workspaces_inter = null;
                this._clients_inter = null;
                this._monitors_inter =  null;
                this._initial_tree_parse = false;

                // Just do it all at once, and don't let anything read while we're changing
                this.notify('clients');
                this.notify('monitors');
                this.notify('workspaces');
                break;
            case 'output':
                this._monitors_inter.set(node.id, node);
                if (node.active)
                    this._active.monitor.update(node.id, node.name);
                node.nodes.map(n => this._handleTreeMessage(n));
                break;
            case 'workspace':
                this._workspaces_inter.set(node.name, node);
                ...
                node.nodes.map(n => this._handleTreeMessage(n));
                if (this._initial_tree_parse) {
                }
                break;
            case 'con':
            case 'floating_con':
                this._clients_inter.set(node.id, node);
                if (node.focused) {
